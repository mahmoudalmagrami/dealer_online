...
class Helper
{
    ...
    {
        $ext = self::getFileExtension($base64);
        $size = self::getFileSize($base64);

        if (!in_array($ext, ['png', 'gif', 'jpeg', 'jpg','pdf'])) {
            throw ValidationException::withMessages(['Unsupported file type']);
        }
        if ($size>10485760) {
            throw ValidationException::withMessages(['The file size is more than 10MB']);
        }
        return true;
    }
    ...
    public static function getFileExtension(string $base64): string
    {
        ...
        return  explode('/', finfo_buffer(finfo_open(), $bin, FILEINFO_MIME_TYPE))[1];
    }
    public static function getFileSize(string $base64): int
    {
        $bin = base64_decode($base64);
        return  strlen($bin);
    }
